...
class TestGetSetIndices:
    ...
    @pytest.mark.parametrize("labels, size, balanced, result", [
    ...
    def test_if_returns_correct_amount(self, labels, size, balanced, result):
        assert len(utils._get_set_indices(labels, size, balanced)) == result
...
class TestTrainValTestSplit:
    ...
    def test_if_sets_overlap(self, ):
        data = np.arange(30)
        train_x, train_y, val_x, val_y, test_x, test_y = utils.train_val_test_split(
            data, self.labels, 0.5, balanced=False)
