...
def main(args):
    ...
    # Init data
    data = Dataset(args.dataset_file, args.gt_file, args.train_samples_per_class,
                   args.neighbours_size)
    ...
    # Build model
    settings = build_settings_for_dataset(args.neighbours_size)
    model = build_model(settings, args.classes_count, data.x_train.shape[-1])
