...
def run_model(args, model, data_prep_function) -> HistoryPack:
    ...
    if args.cont is not None:
        ...
        if cont.endswith('.txt'):
            cont = cont[:-4]
        os.makedirs(os.path.join(args.dest_path, '{}_run_{}_cont_{}'.format(args.data_name,
                                                                            args.run_idx,
                                                                            cont)), exist_ok=True)
        path = os.path.join(args.dest_path, '{}_run_{}_cont_{}'.format(args.data_name,
                                                                       args.run_idx,
                                                                       cont))

    else:
        os.makedirs(os.path.join(args.dest_path, '{}_run_{}'.format(args.data_name,
...
def train_model(model, train_data_loader, val_data_loader, args, path) -> tuple:
    ...
    for epoch in range(int(args.epochs)):
        ...
        val_time_history.append(end - begin)
